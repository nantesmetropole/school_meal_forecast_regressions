...
if __name__ == "__main__":
    ...
    quartiers = sorted(data["Quartier_detail"].unique())

    for quartier in quartiers:
        print(quartier)
        model, X_pred = main(etab=None, quartier=quartier, z_year_score_threshold=args.seuil, alpha=(1-args.int_conf),
                             start_training_date=args.start_training_date, begin_date=args.begin_date,
                                       end_date=args.end_date)
        store_results(name=quartier, model=model, X_pred=X_pred, z_year_score_threshold=args.seuil,
